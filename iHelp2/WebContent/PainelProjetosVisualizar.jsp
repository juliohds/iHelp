	...
	<div class="container-fluid display-table">
		<div class="row display-table-row">
			...
			<div class="col-md-10 col-sm-11 display-table-cell v-align">
				...
				<div class="row">
					<header>
						<h3 style="text-align: center; background-color: #0e1a35; color: white; padding: 5px, 5px, 25px;">Visualizar Projeto | ${projeto.nome}</h3>
						
						<div class="col-md-6">
							<h3 style="text-align: center;">Voluntários Compativeis Não Vinculados</h3>
								...
								<c:forEach var="voluntario" items="${mlista}">
									<tr>
										<td style="display:none">${voluntario.id_voluntario}</td>
										<td>${voluntario.nome}</td>
										<td><a href="javascript:delay('ManterProjeto.do?acao=VincularMatch&id_projeto=${projeto.id_projeto}&id_voluntario=${voluntario.id_voluntario}')"><button type="button" class="btn btn-primary" onclick="Alert.render('Você só pode vincular até a quantidade de voluntarios exigidas pelo projeto ser suficiente')">Deseja Vincular?</button></a></td>
						...
						<div class="col-md-6">
							<h3 style="text-align: center;">Voluntários Compativeis Vinculados</h3>
								...
								<c:forEach var="voluntario" items="${vv}">
									<tr>
										<td style="display:none">${voluntario.id_voluntario}</td>
										<td>${voluntario.nome}</td>
										<td><a href="ManterProjeto.do?acao=DesvincularMatch&id_projeto=${projeto.id_projeto}&id_voluntario=${voluntario.id_voluntario}"><button type="button" class="btn btn-danger">Retirar</button></a></td>
						...
						<div class="col-md-12">
							<div class="alert alert-warning alert-dismissable">
								  ...
								  <Strong>Dica:</Strong> É preciso ter a disponibilidade e área de atuação compativel com a do projeto e você estar entre a quantidade de voluntários necessárias para o projeto para que você seja selecionado			</div>
						</div>
								...
								<div class="form-group">
									<form action="ManterProjeto.do?acao=Atualizar&id_projeto=${projeto.id_projeto}" method="post">
										...
										<select class="form-control input-sm" id="nivel_educacional" name="situacao" required>
									      <option value="" selected>${projeto.situacao}</option>
										  
										  									 				  					
					  					</select>
					  					<br><hr>
										
										<h3>Período das atividades:</h3><br>
										
										<div class="row">
											<label class="col-md-6 control-label" for="">Segunda-feira:
											</label> <input type="checkbox" ${vinculo == '1' ? 'disabled' : ''} id="chkS1" data-toggle="tooltip" title="Manhã" name="chkS1" value="1" ${projeto.disponibilidade[0] == '1' ? 'checked' : ''}>
											<input type="checkbox" ${vinculo == '1' ? 'disabled' : ''} id="chkS2" data-toggle="tooltip" title="Tarde" name="chkS2" value="1" ${projeto.disponibilidade[1] == '1' ? 'checked' : ''}>
											<input type="checkbox" ${vinculo == '1' ? 'disabled' : ''} id="chkS3" data-toggle="tooltip" title="Noite" name="chkS3" value="1" ${projeto.disponibilidade[2] == '1' ? 'checked' : ''}>
										</div>
										...
										<a href="ManterProjeto.do?acao=Carregar"> <button type="button" class="btn btn-primary">Voltar</button></a>
